...
Status MayAddWhileOpToPipelinedGroup(HloInstruction* while_op,
  ...
  for (auto hlo : while_op->operand(0)->operands()) {
    if (hlo->opcode() == HloOpcode::kTuple) {
      // Find the Send in a code pattern like this, assuming that we never
      // copy token data type, which is currently true.
      ///
      //   send = (..., token) send(...)
      //
      //   send.token = token[] get-tuple-element(send)
      //   send.tuple = tuple(..., send.token)
      //   while-init =  tuple(..., send.tuple)
      //   while-result =  while(while-init), ...
      for (auto ele : hlo->operands()) {
        if (ele->shape().IsToken() &&
            ele->opcode() == HloOpcode::kGetTupleElement) {
