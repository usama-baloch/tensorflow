...
TEST_F(AttrsAndConstraintsTest, GetOperandIndexReturnsIndex) {
  OwningOpRef<ModuleOp> module_op = ParseModuleOpString(kModuleMultipleUses);
  ASSERT_TRUE(module_op);
  ...
  auto dot_general_op = *main_fn.getOps<DotGeneralOp>().begin();
  Operation* subtract_op = FindUserOfType<SubtractOp>(dot_general_op);
  EXPECT_THAT(*GetOperandIndex(subtract_op, dot_general_op.getResult()), Eq(1));
}
