...
using ::mlir::quant::GetOperandIndex;
...
Type GetOutputType(Operation* op, MLIRContext& ctx, const bool has_i32_output,
  ...
  // fused `qi8` type.
  Operation* next_op = FindUserOfType<>(uniform_quantize_op);
  next_op->setOperand(
      *GetOperandIndex(next_op, uniform_quantize_op->getResult(0)),
      op->getResult(0));
...
class RewriteQuantizedAddOp : public OpRewritePattern<stablehlo::AddOp> {
 public:
  ...
  void rewrite(stablehlo::AddOp op, PatternRewriter& rewriter) const override {
    rewriter.replaceOpWithNewOp<TFL::AddOp>(op, op.getResult().getType(),
